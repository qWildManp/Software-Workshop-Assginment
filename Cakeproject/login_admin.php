...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($name != "" && $pwd != "") { //if user input all
        ...
        // check user's input from the admin list
        $sqladmin = "SELECT  * FROM adminlist WHERE (name='$name'  AND pwd='$pwd')";
        $resultadmin = $conn->query($sqladmin);
        if ($resultadmin->num_rows > 0) { //if input match the data
            session_start();
            $_SESSION['adminName'] = $name;
            // jump to the admin website
            echo "<script language='javascript'>";
            echo "document.location='admin.php'";
            echo "</script>";
        } else {//if input doesn't match
            $loginErr = "Login failed : incorrect password OR admin name doesn't exist!";
        }
        $conn->close();
...
<form class="form-signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
    ...
    <div>
        ...
        <h4 class="h5 mb-0 font-weight-light">For Admin </h4>
